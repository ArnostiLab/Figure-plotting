master <- read.csv("PlateRatesFinal24h_shelf1234.csv",row.names=1,header=TRUE)
...
levels(master$substrate) = c("a","b","L","AAF","AAPF","QAR","FSR")

#barplot
png('../figures/Plate_barplot_24h.png',width=1000,height=600)
...
plot1 <- plot + coord_cartesian(ylim=c(0,85)) + scale_fill_manual(name=substrate,values=substrateColors) + theme_bw() + theme(strip.text=element_text(size=16),axis.text.x = element_text(angle = 70, hjust = 1,size=16),axis.text.y=element_text(size=16),axis.title.y=element_text(size=20,vjust=2),axis.title.x=element_text(size=20),legend.position="none",strip.background=element_blank(),strip.text.x=element_text(face="bold"),strip.text.y=element_text(face="bold"))  + labs(y="hydrolysis rate (nM/hr)")
print(plot1)
dev.off()

png('../figures/Plate_barplot_24h_smallaxis.png',width=1000,height=600)
plot <- ggplot(master,aes(x=substrate,y=avg_potential_rate)) + geom_bar(aes(fill=substrate),color="gray30",stat="identity") + facet_grid(depthid~stn) + geom_errorbar(aes(ymin=avg_potential_rate-avg_sd,ymax=avg_potential_rate+avg_sd),color="grey30",width=0.5) 
plot1 <- plot + coord_cartesian(ylim=c(0,8)) + scale_fill_manual(name=substrate,values=substrateColors) + theme_bw() + theme(strip.text=element_text(size=16),axis.text.x = element_text(angle = 70, hjust = 1,size=16),axis.text.y=element_text(size=16),axis.title.y=element_text(size=20,vjust=2),axis.title.x=element_text(size=20),legend.position="none",strip.background=element_blank(),strip.text.x=element_text(face="bold"),strip.text.y=element_text(face="bold"))  + labs(y="hydrolysis rate (nM/hr)")
print(plot1)
dev.off()
